(function(Drupal) {
  Drupal.behaviors.offcanvasMenu = {
    attach: function (context, settings) {
      // Use context to filter the DOM to only the elements of interest,
      // and use once() to guarantee that our callback function processes
      // any given element one time at most, regardless of how many times
      // the behaviour itself is called (it is not sufficient in general
      // to assume an element will only ever appear in a single context).

      once('offcanvasMenu', 'button.menu__toggle', context).forEach((element) => {

        // Set the onClick listener for the menu toggle button
        element.addEventListener('click', function(e) {


          var target = element.getAttribute("aria-controls");
          var targetElement = document.getElementById(target);

          // Get the state of the toggle button
          var state = targetElement.getAttribute("aria-expanded");

          // Set an expanded class on the parent menu item "li.menu__item--expanded"
          // to allow for styling of the expanded menu item
          targetElement.parentElement.classList.toggle("menu__item--expanded");
          //$(this).closest('.menu__item').toggleClass('menu__item--expanded');

          // Set the new state of the toggle button, and the state of the menu
          targetElement.setAttribute("aria-expanded", !(state === "true"));
          targetElement.setAttribute("aria-hidden", (state === "true"));
        });
      });
    }
  };
})(Drupal);